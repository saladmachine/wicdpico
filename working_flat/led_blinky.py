...
class LEDBlinkyModule(PicowidModule):
    ...
    def get_dashboard_html(self):
        ...
        <script>
        function toggleLED() {
            setButtonLoading('led-toggle-btn', true);
            serverRequest('/led_toggle')
            .then(result => {
                updateElement('led-status', 'LED Status: ' + result);
                updateLEDState();
            })
            .catch(error => {
                updateElement('led-status', 'Error: ' + error.message);
            })
            .finally(() => {
                setButtonLoading('led-toggle-btn', false);
            });
        }
        
        function updateLEDState() {
            serverRequest('/led_status', 'GET')
            .then(state => {
                updateElement('led-state', state);
            });
        }
